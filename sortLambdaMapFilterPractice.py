...
b_countries = filter(lambda st: st.startswith('B'), countries)
...
passed = [gr[0] for gr in students if gr[1]>=70]
...
l1 = ['left', 'up', 'front']
l2 = ['right', 'down', 'back']
opposites = [l for l in filter(lambda x: len(x[0]) > 3 and len(x[1])>3, zip(l1, l2))]
#7.Below, we have provided a species list and a population list. Use zip to combine these lists into one list of tuples called pop_info. From this list, create a new list called endangeredthat contains the names of species whose populations are below 2500.
